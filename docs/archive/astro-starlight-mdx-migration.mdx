...
> Archived: this migration plan is retained for project history only and is not part of the published website.

## 状态

| 项目 | 状态 |
| --- | --- |
| 方案阶段 | Phase 5 Completed |
| 设计基线 | 首页 HTML 设计稿曾作为首版视觉基线；`design-drafts/` 已在迁移完成后清理 |
| 技术栈决策 | 倾向采用 Astro + Starlight + MDX |
| 迁移准备 | Completed：[Phase 0 盘点](./astro-migration-phase-0-inventory.mdx) |
...
| 外部审核 | Completed：各阶段和后续导航/图片压缩调整已完成 Claude + DeepSeek 收口审核，无遗留 P0/P1 |

本文档用于记录 OpenDesk 官网从 Jekyll 站点迁移到 Astro + Starlight + MDX 的方案与执行历史。当前主线已切到 Astro-only，Jekyll/Ruby 构建入口已删除；旧 Jekyll 站点只应存在于历史提交、tag 或发布分支中。

## 背景

迁移前页面仓库是 Jekyll + Minimal Mistakes 主题，内容主要分布在：
...
- 不在迁移初期重写全部文档内容。
- 不在 Astro 内容、路由和部署未验证前删除迁移期 Jekyll 站点。
...
| 迁移前 Jekyll 路径 | 目标 Astro/Starlight 路径 | 处理方式 |
| --- | --- | --- |
...
`design-drafts/homepage-v1.html` 曾作为首版视觉模板。迁移时没有长期保留临时 HTML，而是拆分为稳定组件：
...
- 已在迁移期 Jekyll `_config.yml` 中排除 `src`、`public`、`dist`、`node_modules` 和 Astro 配置文件，避免 Jekyll 解析 `.astro` front matter；
- Jekyll 站点在迁移期临时保持可构建，用于当时的线上回退；这不是最终状态，Phase 5 必须删除 Jekyll/Ruby 构建入口。

验收标准：

- `pnpm build` 成功；
- 迁移期 Jekyll 构建命令仍可成功执行，作为回退检查，不作为最终主线要求；
...
- Lighthouse 基线当时留到 Phase 5 切流前执行，避免在内容和资源仍迁移时产生误导性指标。
...
- 迁移前 Jekyll permalink 列表有对应 Astro 路由或 redirect。
...
### Post-Migration：导航与图片压缩收口

状态：Completed，详见 [2026-06-09 Post-Migration Site Updates](./2026-06-09-post-migration-site-updates.md)

- 顶部导航 `快速开始` 改为 `使用文档`，并通过 `activePath: '/docs/'` 保持所有 `/docs/*` 页面下的稳定 active 状态；
- 首页 `快速开始` CTA 继续跳转 `/docs/quickstart/cli/`；
- 首次尝试的激进图片加载优化已回滚，最终只保留静态图压缩和直接路径替换；
- `firsttime_gui.png`、`file_batch.png`、`file_organize.png`、`file_transfer.png` 已替换为压缩 JPG；
- `banner.jpg` 与 `linuxvm.png` 保留原路径并重新压缩；
- 已通过 `pnpm build`、`pnpm check:links`、旧图片引用扫描、Claude + DeepSeek 审核和 GitHub Pages 预览部署验证。
...
| 2026-06-09 | 首页视觉以临时 HTML 设计稿为首版模板，迁移完成后删除 `design-drafts/` | Completed |
...
| 2026-06-09 | Post-migration 修复顶部文档导航 active 状态，并以静态图压缩替代激进图片加载优化 | Completed |

## 下一步

1. 主线合入后继续确认 GitHub Pages workflow 运行成功；
